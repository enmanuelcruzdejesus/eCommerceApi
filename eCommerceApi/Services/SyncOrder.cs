...
namespace eCommerceApi.Services
{
    public class SyncOrder
    {
        ...
        public SyncOrder(RestAPI restAPI)
        {
            _restApi = restAPI;
        ...
        public async Task Sync()
        {
            ...
            //getting the changes
            var syncRecords = AppConfig.Instance().Db.TransSyncLog.Get(x => x.CreatedDate > _syncLastUpdateSync && x.IsSynchronized == false).Count();
            var transactions = db.TransSyncLog.Get(t => t.CreatedDate > _syncLastUpdateSync && t.IsSynchronized == false).ToList();
            ...
            if (syncRecords > 0)
            {
                ...
                var transUpdated = db.TransSyncLog.Get(t => t.TableName == "Orders" && t.Operation == "Update" && t.CreatedDate > _syncLastUpdateSync && t.IsSynchronized == false).ToList();
                ...
                var updatedOrders = from i in db.Orders.GetAll()
                ...
                //updating last sync
                db.SyncTables.Update(new SyncTables() { UserId = 100, LastUpdateSync = DateTime.Now }, s => s.UserId == 100 && s.TableName == "Orders");



                var t = (from x in AppConfig.Instance().Db.TransSyncLog.Get(x => x.CreatedDate > _syncLastUpdateSync)
